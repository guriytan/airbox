...
func (r *RedisClient) GetCaptcha(ctx context.Context, email string) string {
	return r.client.WithContext(ctx).Get(global.KeyCaptcha + email).Val()
}

func (r *RedisClient) DeleteCaptcha(ctx context.Context, email string) {
	r.client.WithContext(ctx).Del(global.KeyCaptcha + email)
}
...
func (r *RedisClient) GetToken(ctx context.Context, name string) string {
	return r.client.WithContext(ctx).Get(global.KeyToken + name).Val()
}

...
func (f *FileDaoImpl) SelectFileByStorageID(db *gorm.DB, sid string) (files []model.File, err error) {
	err = db.Preload("FileInfo").Where("storage_id = ? and folder_id is null", sid).Order("created_at desc").Find(&files).Error
	return
}
